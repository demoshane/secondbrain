...
- **D-17:** GUI **Intelligence page** gets an "Install Chrome Extension" button that opens `chrome://extensions` and shows the same step-by-step instructions inline. Should also show current connection status (sb-api reachable from browser context) so the user can confirm the extension will work after install.
